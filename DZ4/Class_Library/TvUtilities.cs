...
namespace Class_Library
{
    public static class TvUtilities
    {
        ...
        public static void Sort(List<Episode> episodes)
        {
            int min;
            for (int i = 0; i < episodes.Count - 1; i++)
            {
                ...
                for (int j = i + 1; j < episodes.Count; j++)
                {
        ...
        public static List<Episode> LoadEpisodesFromFile(string FileName)
        {
            ...
            List<Episode> LoadedEpisodes = new List<Episode>(File.ReadAllLines(FileName).Length);
